...
def build_translation_prompt(
    ...
    system = (
        ...
        f"   - Example: <math>1 \\cdot 2 \\cdots k</math> → <math>1 dot.op 2 dots.c k</math>\n"
        ...
        f"   → '<typst>#grid(columns: 1, row-gutter: 4pt, [Hình nón], [$V = pi r^2 h / 3$], [$A = pi r sqrt(r^2 + h^2)$])</typst>'\n"
...
def build_math_fix_prompt(
    ...
    system = (
        ...
        "**Pattern B — One-label-one-or-more-formulas** (portrait bbox only):\n"
        ...
        "Only apply when bbox height >= width (portrait). Always use columns: 1.\n"
        ...
        "  Input:  'Hình nón <math display=\"block\">V = frac(1, 3) pi r^2 h</math> <math>A = pi r sqrt(r^2 + h^2)</math>'\n"
        "  Output: <typst>#grid(columns: 1, row-gutter: 4pt, [Hình nón], [$V = pi r^2 h / 3$], [$A = pi r sqrt(r^2 + h^2)$])</typst>\n"
