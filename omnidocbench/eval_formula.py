...
Ví dụ
-----
    python eval_formula.py \
        --gt ../../OmniDocBench.json --pred ../../parser_json \
        --mapping ../../mapping.json --out ../../eval_report_formula.json
"""
...
def norm_formula(s: str) -> str:
    """Bóc lớp bọc (math tag + delimiter) -> LaTeX trần, gom khoảng trắng."""
    s = _MATH_TAG.sub(" ", s or "").strip()
    changed = True
    while changed:                      # bóc nhiều lớp lồng nhau nếu có
        changed = False
        for a, b in _DELIMS:
...
def score_page(gts, preds, member_thr):
    ...
    for g in gts:
        ...
        if gt_norm or pred_norm:
            d = E._dist(pred_norm, gt_norm)
            ps["pairs"].append((d, max(len(pred_norm), len(gt_norm)), matched))
