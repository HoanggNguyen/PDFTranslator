...
Appendix~\ref{app:e2e-benchmark} gives all seven quality measures with 95\%
confidence intervals. Our system has the highest immutable-object
...
($\tau=0.981$ versus $0.985$), and its COMETKiwi estimate is statistically tied
with BabelDOC ($0.695$ versus $0.697$). Its observed runner time is $3.6$ seconds
per input page, compared with $4.1$ for BabelDOC, $3.2$ for PDFMathTranslate and
$2.2$ for DeepL Document.

\begin{table}[H]
\centering
\caption{Selected end-to-end results on 204 English-to-Vietnamese pages. Full
scores and intervals are in Table~\ref{tab:app-e2e-full}.}
...
annotated element (OF-harm $0.032$ versus $0.018$ and $0.020$). The much lower
IC-harm indicates that this overflow seldom becomes merged visible ink, which
is consistent with expanding into available whitespace, but OF-harm remains a
reconstruction limitation. Our system leaves $0.387$ untranslated English
blocks per page, statistically indistinguishable from BabelDOC's $0.240$ and
substantially below PDFMathTranslate's $2.333$. DeepL changes the page count in
two of six documents, so its page-anchored visual scores cover the remaining
four; this reflow is reported rather than silently aligned.
