...
The experiments test H1 and H2 as stage-level hypotheses, not as a claim of
end-to-end superiority. Translation is measured on WMT24++ (M2, M3), while
structure analysis is measured on OmniDocBench (M1). We do not run a direct
system-level baseline in this version because the closest tools expose different
contracts: some target born-digital PDFs, some export Markdown/JSON rather than
a translated PDF overlay, and some are documented mainly as source repositories.
A fair comparison requires a shared PDF corpus, target-language setting and
metrics for both adequacy and final visual fidelity.
...
\texttt{numpy}. CDM was trusted only after a ground-truth-against-itself sanity
...
\paragraph{Pilot end-to-end profiling.} To expose the operational profile of
the implementation, we also ran the complete
\texttt{parse}\,$\to$\,\texttt{translate}\,$\to$\,\texttt{render} pipeline on
seven held-out PDFs (207 pages). This set is unannotated and not used as a
quality benchmark.
